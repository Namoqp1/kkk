...
for i,v in pairs(game:GetService("Players").LocalPlayer.PlayerGui.HUD.Inventory.ScrollingFrameList:GetChildren()) do
	if v.Name ~= "UIGridLayout" then
		if not kikiset[v.Name] then
			table.insert(kiki, v.Name)
			kikiset[v.Name] = true
		end	
	end
end
...
ez:OnChanged(function(ezs)
	_G.SelectedStore = {}
	for lol, nig in pairs(ezs) do
		if nig then
			table.insert(_G.SelectedStore, lol)
		end
	end
end)
...
Store:AddToggle("Auto Store One", {
	...
	while task.wait() do 
		if _G.AutoStoreFruitone then
			pcall(function()
				...
				for i,v in pairs(game.Players.LocalPlayer.Backpack:GetChildren()) do 
					if v:IsA("Tool") then
						if v.Name:lower():find(table.find(_G.SelectedStore, v.Name)) then 
